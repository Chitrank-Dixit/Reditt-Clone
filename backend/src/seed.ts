...
const seedDatabase = async () => {
  try {
    ...
    const users = await User.create([
      { name: 'dev_user', email: 'dev@example.com', password: 'password123', bio: 'Full-stack developer with a passion for building cool things with Node.js and React.', avatarUrl: 'https://picsum.photos/seed/dev_user/200', karma: 2450 },
      { name: 'react_fan', email: 'react@example.com', password: 'password123', bio: 'Frontend enthusiast. I love everything about the React ecosystem.', joinDate: new Date('2022-08-15T10:00:00.000Z'), karma: 850 },
      { name: 'tailwind_guru', email: 'tailwind@example.com', password: 'password123', bio: 'Making the web beautiful, one utility class at a time.', avatarUrl: null, joinDate: new Date('2021-03-22T14:30:00.000Z'), karma: 1200 },
    ]);
    ...
    const posts = await Post.insertMany([
      {
        ...
        author: { id: users[0].id, name: users[0].name },
        subreddit: 'reactjs',
        votes: 1200,
        commentsCount: 2,
      ...
      },
      {
        ...
        author: { id: users[2].id, name: users[2].name },
    ...
    await Comment.insertMany([
        {
            ...
            author: { id: users[1].id, name: users[1].name },
            votes: 55,
            post: posts[0]._id,
            replies: [reply._id],
        },
